<?php
session_start();
function createVideoList($conn, $vId, $vType, $usersId, $vName, $genres) {
    $sql = "INSERT INTO video (vId, vType, vGenre,usersId) VALUES (?, ?, ?, ?);";
    ...
    if(checkRepeat($conn, $vId, $usersId)){
        for ($i=0; $i < count($genres); $i++) { 
            mysqli_stmt_bind_param($stmt, "ssss", $vId, $vType, $genres[$i], $usersId);
            mysqli_stmt_execute($stmt);
            
            if ($i === (count($genres) - 1)){
                createVideoPost($conn, $vName, $usersId);
            }
        }
