...
if ($is_following) {
    ?><button name="unfollow" id="submit-btn" style="margin: 0 10px;" class="btn btn-primary btn-primary">Unfollow</button> <?php
} else {
    ?><button name="follow" id="submit-btn" style="margin: 0 10px;" class="btn btn-primary btn-info">Follow</button> <?php
}
...
if (isset($_POST["follow"])) {
    $sql_s = "INSERT INTO follows (currentUserId, followedUserId) VALUES ($userid, $var)";
    $result_s = $conn->query($sql_s);
    $sql_s = "UPDATE users SET `follower`=`followers`+1 WHERE `usersId`=$var";
    $result_s = $conn->query($sql_s);
    $sql_s = "UPDATE users SET `following`=`following`+1 WHERE `usersId`=$userid";
    $result_s = $conn->query($sql_s);
}

if (isset($_POST["unfollow"])) {
    $sql_u = "DELETE FROM follows WHERE currentUserId=$userid AND followedUserId=$var";
    $result_u = $conn->query($sql_u);
    $sql_u = "UPDATE users SET `follower`=`followers`-1 WHERE `usersId`=$var";
    $result_u = $conn->query($sql_u);
    $sql_u = "UPDATE users SET `following`=`following`-1 WHERE `usersId`=$userid";
    $result_u = $conn->query($sql_u);
}
