<?php
    ...
    $sql_f = "SELECT * FROM `follows` WHERE `currentUserId`=$var";
    ...
    if ($result_new->num_rows > 0) {
        ...
        // 顯示使用者名稱
        $currentUserId = $row_f["currentUserId"];
        $followedUserId = $row_f["followedUserId"];
    } 
    ...
    function check_if_user_is_following($current_user_id, $user_id_to_follow) {
         // 在此處檢查是否已經追蹤該用戶，例如：
        // 從數據庫中檢索關於當前用戶和該用戶的追蹤狀態，並返回true或false
        // 在這個例子中，我們只是假設當前用戶是否已經追蹤了該用戶
        foreach ($row_f as $row_f){
            if($var = $row_f["followedUserId"]){
                return true;
            }else{
                return false;
            }
                                ...
                                .background-img {
                                    ...
                                    <?php
                                        ...
                                        if($current_user_id != $user_id_to_follow){
                                            $is_following = check_if_user_is_following($current_user_id, $user_id_to_follow);
                                            // 根據結果顯示follow或unfollow
                                            if ($is_following) {
                                                ?><button>Unfollow</button> <?php
                                            } else {
                                                ?><button>Follow</button> <?php
                                            }
