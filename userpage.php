...
if ($result_new->num_rows > 0) {
    ...
    $posts = $row_new["posts"];
    $followers = $row_new["followers"];
    $followings = $row_new["following"];
} else {
    echo "查無此使用者";
}
...
if (isset($_POST["follow"])) {
    ...
    header("Location:http://localhost/db_project/userpage.php?varname=$var#");
    // check_if_user_is_following($current_user_id, $user_id_to_follow, $conn_new) == true;
}
...
if (isset($_POST["unfollow"])) {
    ...
    header("Location:http://localhost/db_project/userpage.php?varname=$var#");
    // check_if_user_is_following($current_user_id, $user_id_to_follow, $conn_new) == false;
}

function check_if_user_is_following($current_user_id, $user_id_to_follow, $conn_new) {
    // echo "<script>alert('$current_user_id');</script>";
    ...
    if ($result_f && mysqli_num_rows($result_f) > 0) {
        // echo "查詢到 $result_f->num_rows 筆資料";
        // echo "<script>alert('follow');</script>";
        return true;        
    } else {
        // var_dump($result_f);
...
<?php
    ...
    $sql = "SELECT DISTINCT vId, comment FROM video WHERE usersId = '$var'";
                            ...
                            <div class="rela-block container">
                                ...
                                .background-img {
                                    ...
                                    <div class="rela-block profile-card-stats">
                                        <?php $sql = "SELECT COUNT(*) as count FROM posts WHERE `usersId` = $var";
                                            $stmt = $db->prepare($sql);
                                            $stmt->execute();
                                            $countpost = $stmt->fetchColumn();
                                        ?>
                                        <div class="floated profile-stat works" id="num_works"><?php echo $countpost?><br></div>
                                        <a href="all_followers_user.php?varname=<?php echo $var ?>#" class="floated profile-stat followers h" id="num_followers"><?php echo $followers?><br></a>
                                        <a href="all_following_user.php?varname=<?php echo $var ?>#" class="floated profile-stat following h" id="num_following"><?php echo $followings?><br></a>
                                        ...
                                        <form action="" method="post" style="display: flex;justify-content: center;">
                                            <?php
                                                ...
                                                if($current_user_id != $user_id_to_follow){
                                                    if (check_if_user_is_following($current_user_id, $user_id_to_follow, $conn_new) == true) {
                                ...
                                <div class="rela-block content"style = "display:flex">
                                    <div class="postBlock" style="display:none;">
                                        ...
                                        <div class="">
                                            ...
                                            <div class="">
                                                ...
                                                <section >
                                                <?php
                                                $allpost = $db->query("SELECT * FROM `posts` WHERE `usersId`= $var ORDER BY `post_date`  DESC");
                                                                ...
                                                                <ul class="list-group list-group-horizontal">
                                                                    <li class="list-group-item flex-fill text-center p-0 px-lg-2 border border-0">
                                                                        <form method="POST" action="postlike.php">
                                                                            <a class="small text-decoration-none"type="submit" name="userpagelike" href="#">

                                                                                <input type="hidden" name="post_id" value=<?php echo $all['postId']?>>
                                                                                <input type="hidden" name="post_userid" value=<?php echo $all['usersId']?>>
                                                                                <?php
                                                                                    $usersId = $_SESSION['userid'];
                                                                                    ...
                                                                                    <button onclick="saveScrollPosition()" type="submit" name="userpagelike" style="background-color: transparent; border: none; color: <?php echo $hasLiked ? '#1c145d' : '#4e73df'; ?>; cursor: pointer; <?php echo $hasLiked ? 'font-weight:bold' : ''; ?>">
                                    ...
                                    <div class="videoBlock" style="display:none;">
                                        ...
                                        // 建立 SQL 查詢字串，查詢使用者名稱
                                        $sql_video = "SELECT DISTINCT vId, vType, comment FROM video WHERE usersId = '$var'";

                                        $result = mysqli_query($conn, $sql_video);

                                        // 將查詢結果轉換成陣列
                                        $video_rows = array();
                                        while ($video_row = mysqli_fetch_assoc($result)) {
...
<script>
        var video = <?= json_encode($video_rows)?>
        // console.log(video);
